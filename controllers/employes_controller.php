...
class EmployesController extends AppController {
	...
	function add() {
		...
		$roles = $this->Employe->Role->find('list');
		$archivedTransportations = $this->Employe->ArchivedTransportation->find('list');
		$trains = $this->Employe->Train->find('list');
		$this->set(compact('roles', 'archivedTransportations', 'trains'));
	}
